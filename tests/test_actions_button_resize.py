...
- #116: `double_click` was refused on a desktop/nested target as browser-only, and two rapid
  `click`s don't coalesce into an OS-level dblclick. It is the click primitive with `count=2` —
  same targeting (x,y / ref / element), same `button`, reported as "double-clicked".
"""
...
# --- #116: double_click on the desktop = the click primitive, count=2 -------------------------
