...
def _el(index, x, y, w, h, name="btn"):
    from interact.desktop import DesktopElement

    return DesktopElement(index=index, role="button", name=name, x=x, y=y, w=w, h=h)


@pytest.mark.asyncio
async def test_refine_dense_strips_finds_new_elements(srv):
    """Strip pass discovers elements missed by full-frame pass."""
    # Full-frame elements: one in top strip (y=10), rest in center
    # Image is 800x1400 so strip_h = 112 (>100 minimum)
    initial = [_el(1, 50, 10, 40, 20), _el(2, 200, 600, 80, 30)]
    # Strip VLM returns a new small element in the top strip
    strip_json = '[{"role":"button","name":"Menu","x":400,"y":50,"w":80,"h":30}]'
    mock_vlm = AsyncMock(return_value=VLMResult(text=strip_json, elapsed=0.1))
    with (
        patch.object(srv, "_vlm", mock_vlm),
        patch.object(srv, "_crop_image", return_value=_PNG),
    ):
        result = await det._refine_dense_strips(_PNG, "ctx", 800, 1400, initial)
    assert len(result) > len(initial)
    assert any(el.name == "Menu" for el in result)
    assert result[-1].index == len(result)


@pytest.mark.asyncio
async def test_refine_dense_strips_skips_when_no_edge_elements(srv):
    """No refinement when all elements are in the center."""
    # Image 800x1400, strip_h=112. All elements in vertical center.
    center_els = [_el(1, 100, 400, 60, 30), _el(2, 300, 600, 60, 30)]
    result = await det._refine_dense_strips(_PNG, "ctx", 800, 1400, center_els)
    assert result is center_els  # unchanged, same object


@pytest.mark.asyncio
async def test_refine_dense_strips_deduplicates(srv):
    """Overlapping elements from strip pass are filtered by IoU."""
    # Image 800x1400, strip_h=112. Element at top triggers refinement.
    initial = [_el(1, 50, 10, 40, 20, "Save")]
    # Strip VLM returns an element that overlaps heavily with the existing one
    strip_json = '[{"role":"button","name":"SaveDup","x":50,"y":10,"w":40,"h":20}]'
    mock_vlm = AsyncMock(return_value=VLMResult(text=strip_json, elapsed=0.1))
    with (
        patch.object(srv, "_vlm", mock_vlm),
        patch.object(srv, "_crop_image", return_value=_PNG),
        ...
        result = await det._refine_dense_strips(_PNG, "ctx", 800, 1400, initial)
    # Duplicate should be filtered -- count stays at 1
    assert len(result) == 1
    assert result[0].name == "Save"


@pytest.mark.asyncio
async def test_refine_dense_strips_skips_small_images(srv):
    """Strip refinement skips when strips would be < 100px tall."""
    initial = [_el(1, 50, 10, 40, 20)]
    result = await det._refine_dense_strips(_PNG, "ctx", 800, 600, initial)
...
@pytest.mark.asyncio
async def test_refine_quadrants_merges_new_elements(srv):
    """Quadrant pass discovers elements missed by full-frame pass, deduplicates by IoU."""
    initial = [_el(1, 50, 50, 40, 20, "Existing")]
    # Quadrant VLM returns one new element (non-overlapping) and one duplicate
    new_json = '[{"role":"button","name":"New","x":200,"y":150,"w":60,"h":30}]'
    dup_json = '[{"role":"button","name":"ExDup","x":50,"y":50,"w":40,"h":20}]'
    call_count = 0

    async def _vlm_side_effect(*args, **kwargs):
        nonlocal call_count
        call_count += 1
        # First quadrant returns new element, second returns duplicate, rest empty
        if call_count == 1:
            return VLMResult(text=new_json, elapsed=0.1)
        if call_count == 2:
            return VLMResult(text=dup_json, elapsed=0.1)
        return VLMResult(text="[]", elapsed=0.1)

    mock_vlm = AsyncMock(side_effect=_vlm_side_effect)
    with (
        patch.object(srv, "_vlm", mock_vlm),
        patch.object(srv, "_crop_image", return_value=_PNG),
    ):
        result = await det._refine_quadrants(_PNG, "ctx", 800, 600, initial)
    ...
    assert [el.index for el in result] == list(range(1, len(result) + 1))


@pytest.mark.asyncio
async def test_refine_quadrants_tolerates_failures(srv):
    """Quadrant exceptions are logged, not raised."""
    initial = [_el(1, 50, 50, 40, 20)]
    mock_vlm = AsyncMock(side_effect=RuntimeError("network error"))
    with (
        patch.object(srv, "_vlm", mock_vlm),
        patch.object(srv, "_crop_image", return_value=_PNG),
    ):
    ...
    # Original elements preserved despite all quadrants failing
    assert len(result) == 1
    assert result[0].name == "btn"
