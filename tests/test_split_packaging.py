...
def test_root_build_configuration_targets_only_the_new_python_packages() -> None:
    ...
    assert configuration["tool"]["hatch"]["build"]["targets"]["wheel"]["packages"] == [
        ...
        "packages/interact-contracts/src/interact_contracts",
    ]
...
def test_root_wheel_contains_both_public_imports_and_the_interact_cli(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        assert any(name.startswith("interact_contracts/") for name in names)
        entry_points, = (name for name in names if name.endswith(".dist-info/entry_points.txt"))
        assert "interact = interact.cli:main" in archive.read(entry_points).decode()
    subprocess.run(
        [sys.executable, "-I", "-c", "import sys; sys.path.insert(0, sys.argv[1]); import interact, interact_contracts", str(wheel)],
        cwd=tmp_path,
        check=True,
    )
...
def test_contract_package_builds_without_the_local_runtime(tmp_path: Path) -> None:
    subprocess.run(
        ["uv", "build", "--wheel", "--project", "packages/interact-contracts", "--out-dir", str(tmp_path)],
        cwd=ROOT,
        check=True,
        env=os.environ | {"UV_OFFLINE": "1", "PYTHONDONTWRITEBYTECODE": "1"},
    )
    wheel, = tmp_path.glob("interact_contracts-*.whl")
    with zipfile.ZipFile(wheel) as archive:
        ...
        assert "interact_contracts/__init__.py" in archive.namelist()
