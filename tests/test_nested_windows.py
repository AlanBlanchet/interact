"""list_windows filters out toolkit utility windows.

A WM-less nested display hosts utility windows real apps create alongside their UI — Qt's
1x1 "Qt Selection Owner for <app>" — and during a slow startup (a numba/JIT warm) those are
the ONLY windows for tens of seconds. list_windows reporting them made launch_app's poll
latch on and advertise `target="nested:Qt Selection Owner for pagb"` as the drive target
while the real window was still mapping (real PAGB sessions). Tiny utility windows are
never drive targets.
"""
