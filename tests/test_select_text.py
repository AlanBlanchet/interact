"""select_text (#32): actuate a selection-gated control (e.g. a Lexical inline toolbar).

`drag` dispatches HTML5 drag-and-drop, not a text selection, and two clicks don't coalesce into
a dblclick — `double_click` (validated and dispatched in `test_double_click.py`) is the OTHER
way to reach a selection-gated control. Validation here is pure; the DOM-selection behaviour is
checked live, proving BOTH actions actually open a DOM Selection in a contenteditable.
"""
