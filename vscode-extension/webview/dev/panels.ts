 ...
 *
 *  This used to live in `/tmp`, written by hand during a review. Four tests depended on it, so when
 *  `/tmp` was cleaned they stopped running — silently, reported as "the chat render fixture is not
 *  present", which reads like a deliberate skip rather than a guard that has quietly gone. A test
...
function standalone(html: string, theme: string): string {
  ...
  return html
    ...
    // WITH the nonce: the document carries `script-src 'nonce-…'`, so an un-nonced shim is blocked
    // by the page's own CSP, `acquireVsCodeApi` stays undefined, and the real script throws on its
    // first line — every handler unbound, which looks exactly like a broken composer.
