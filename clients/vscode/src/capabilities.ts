 ...
 *  a browser, spawn other agents and spend money. A team where you cannot see who can do what is
...
/** Which faculty a tool name grants, or null when we do not recognise it.
 *
 *  Unknown grants NOTHING, deliberately. A vendor adding a tool must never silently hand a
 *  character a power it does not have — on this axis, guessing high is the dangerous direction.
 */
function facultyOf(tool: string): string | null {
  ...
  // interact's own surface: eyes and hands on a real screen. `report_issue` is not a faculty,
  // it is paperwork — it grants nothing so it does not dress a character up as an operator.
 ...
 *
 *  Frontmatter only, because the body of a definition is prose that routinely mentions tool names
 *  and even example `tools:` lines — parsing those would grant powers the agent never had.
 */
